...
# # הגדרת היסט (Offset) בפיקסלים לתיקון הסטייה
# OFFSET_X = 15  # שנוי ערך זה (למשל 5, 10, -5) עד להתאמה מושלמת
# OFFSET_Y = 10  # שנוי ערך זה (למשל 5, 10, -5) עד להתאמה מושלמת
# PIECE_OFFSETS = {
#     "K": (0, 0),    # מלך
#     "Q": (0, 0),    # מלכה
#     "R": (0, 0),    # צריח
...
#         self.board = Img().read("assets/board.png")
#         self.pieces = {
#             ("w","K"): Img().read("assets/pieces_mine/wK/states/idle/sprites/1.png", size=(100, 100)),
#             ("w","Q"): Img().read("assets/pieces_mine/wQ/states/idle/sprites/1.png", size=(100, 100)),
#             ("w","R"): Img().read("assets/pieces_mine/wR/states/idle/sprites/1.png", size=(100, 100)),
#             ("w","B"): Img().read("assets/pieces_mine/wB/states/idle/sprites/1.png", size=(100, 100)),
#             ("w","N"): Img().read("assets/pieces_mine/wN/states/idle/sprites/1.png", size=(100, 100)),
#             ("w","P"): Img().read("assets/pieces_mine/wP/states/idle/sprites/1.png", size=(100, 100)),

#             ("b","K"): Img().read("assets/pieces_mine/bK/states/idle/sprites/1.png", size=(100, 100)),
#             ("b","Q"): Img().read("assets/pieces_mine/bQ/states/idle/sprites/1.png", size=(100, 100)),
#             ("b","R"): Img().read("assets/pieces_mine/bR/states/idle/sprites/1.png", size=(100, 100)),
#             ("b","B"): Img().read("assets/pieces_mine/bB/states/idle/sprites/1.png", size=(100, 100)),
#             ("b","N"): Img().read("assets/pieces_mine/bN/states/idle/sprites/1.png", size=(100, 100)),
#             ("b","P"): Img().read("assets/pieces_mine/bP/states/idle/sprites/1.png", size=(100, 100)),
#         }
...
#         frame = Img()
#         # FIX: Initialize the frame with the background image, not the board
#         frame.img = self.background.img.copy()

#         # Overlay the board onto the center of the background
#         board_h, board_w = self.board.img.shape[:2]
#         frame.img[self.start_y:self.start_y+board_h, self.start_x:self.start_x+board_w] = self.board.img
        
#         # 1. Draw selection square
#         if selected_pos:
#             sel_x, sel_y = selected_pos
#             # Account for the board's offset relative to the background
...
#             # B. Check for active jump animation
...
#             # Draw the piece
#             sprite = self.pieces.get((piece_obj.color, piece_obj.type))
#             if sprite:
#                 sprite.draw_on(frame, x_pixel, y_pixel + y_offset)
...
class Renderer:
    ...
    def render(self, board_obj, selected_pos=None):
        ...
        for piece_obj in board_obj.grid.values():
            ...
            piece_sprite_np = self.sprite_manager.get_piece_sprite(piece_obj.id, current_state)
